...
**目标**: 定义停车场领域全部 TypeScript 类型和 Zod schema。

**文件**:
- `src/types/index.ts` — 新增 ParkingLot、ParkingLotStats、Lane、LaneDevice 等业务类型
...
export const createParkingLotSchema = z.object({
  ...
  totalSpots: z.number().int().positive("总车位数必须大于0"),
...
2. **`mockParkingLotStats`**: 聚合统计数据对象，从 mockParkingLots 汇总计算

3. **`mockLanes`**: 按 parkingLotId 索引的 lane 数据，每个停车场 2-6 个 lane，部分设备离线

4. **辅助函数**: `getParkingLotById(id)`, `getLanesByParkingLotId(lotId)`
...
遵循 `src/lib/api/auth.ts` 的服务对象模式：
...
export const parkingLotService = {
  getStats(): Promise<ParkingLotStats> {
    return apiClient.get("/api/parking-lots/stats");
  },

  getList(params?: { page?: number; pageSize?: number; keyword?: string }): Promise<ParkingLot[]> {
  ...
  getLanes(parkingLotId: string): Promise<Lane[]> {
...
**验收**: 接口签名与 PRD Mock API 表对齐，TypeScript 无编译错误。
...
| Method | Path | 行为 |
|--------|------|------|
| GET | `/api/parking-lots/stats` | 返回聚合统计，300ms delay |
| GET | `/api/parking-lots` | 支持分页参数和 keyword 搜索过滤，300ms delay |
| POST | `/api/parking-lots` | 创建停车场（生成新 ID + mock 数据），600ms delay |
| PATCH | `/api/parking-lots/:id` | 更新停车场信息，300ms delay |
| GET | `/api/parking-lots/:id/lanes` | 按 ID 返回车道列表，200ms delay |
| PUT | `/api/parking-lots/:id/lanes` | 更新车道配置，600ms delay |

所有 handler 从 `src/mocks/data/parking-lots.ts` 读取数据。PUT lanes 支持新增和修改（根据有无 id 区分）。
...
打开时加载 `parkingLotService.getLanes(id)` 获取车道数据。
...
数据获取模式：
- 使用 `useEffect` + `useState` 在组件挂载时并行加载 stats 和 list
- 搜索防抖 300ms
